...
export const VOXEL_PROMPT = "I have provided an image. Code a beautiful voxel art scene inspired by this image. Write threejs code as a single-page. IMPORTANT: Include a subtle ambient animation such as a gentle floating (bobbing up and down) or a very slow rotation of the main subject.";
...
export const generateVoxelScene = async (
  ...
  if (palette && palette.length > 0) {
    customPrompt += `\n\nSTRICT COLOR REQUIREMENT: You MUST primarily use this color palette: ${palette.join(', ')}. Do not deviate significantly from these colors.`;
  }
  ...
  try {
    ...
    console.error("Voxel scene failed:", error);
    throw error;
  }
};
